...
CREATE POLICY "Participants can view lists" ON lists
  FOR SELECT USING (
    ...
    public.is_list_participant(lists.id, auth.uid())
  );
...
CREATE POLICY "Participants can view participants" ON list_participants
  FOR SELECT USING (
    public.is_list_participant(list_participants.list_id, auth.uid())
  );
...
-- Function to get user lists (owned + participated)
CREATE OR REPLACE FUNCTION public.get_user_lists(target_user_id UUID)
RETURNS TABLE (
  id UUID,
  yjs_document_id TEXT,
  owner_id UUID,
...
BEGIN
  ...
  SELECT 
    ...
    l.updated_at
  FROM lists l
  WHERE (l.owner_id = target_user_id OR l.id IN (
    SELECT lp.list_id 
